...
<head>
    ...
    <link rel="stylesheet" href="new_things.css">
...
<body>
    ...
    <!-- Affiche les 6 derniers claviers ajoutés à la base de données -->
    <?php
    // Connexion à la base de données
    $db = new PDO('mysql:host=mysql-chinchilla.alwaysdata.net;dbname=chinchilla_bdd;charset=utf8', '345543', 'chinchilla@8520');

    // Préparation de la requête SQL pour récupérer les 6 derniers claviers ajoutés
    $sql = "SELECT * FROM Produit ORDER BY date DESC LIMIT 6";
    ...
    if ($result->rowCount() > 0) {
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<p>Clavier ajouté le : ' . $row['date'] . '</p>';
            echo '<hr class="ligne-separatrice">';
